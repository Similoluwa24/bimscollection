...
function ProductItems({ product_items_prop }) {
  const { product, addToCart } = useContext(EcomContext);
  const productItem = product.find((items)=>items._id === product_items_prop._id)

  
  return (
    <div>
  <div className="m-4 border-[#043040] card">
  ...
  <button onClick={()=>addToCart(product_items_prop._id, 1, productItem)}>
